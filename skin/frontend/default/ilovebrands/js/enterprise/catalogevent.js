...
Object.extend(Enterprise.CatalogEvent.Ticker.prototype, {
    ...
    applyTimer: function () {
        ...
        if (daySec > 0) {
			...
			this.container.down('.second').next('.delimiter').hide();
...
Enterprise.CatalogEvent.TimeSlide = Class.create();

Object.extend(Enterprise.CatalogEvent.TimeSlide.prototype, {
    initialize: function (container, seconds) {
        this.container = $(container);
        this.seconds   = seconds;
        this.start     = new Date();
    ...
    applyTimer: function () {
		...
		var timerWidth = this.container.getWidth();
		//var day = Math.floor(daySec/(3600*24));
		var hour = Math.floor((hourSec - daySec)/3600);
		var min = Math.floor((minuteSec - hourSec)/60);
		var sec = seconds - minuteSec;
		
		//this.container.down('.timer-days').setStyle({width: ((timerWidth/31)*day)+15 + 'px'});
		this.container.down('.timer-hours').setStyle({width: ((timerWidth/24)*hour)+15 + 'px'});
		this.container.down('.timer-minutes').setStyle({width: ((timerWidth/60)*min)+15 + 'px'});
		this.container.down('.timer-seconds').setStyle({width: ((timerWidth/60)*sec)+15 + 'px'});
    },
    formatNumber: function (number) {
        if (number < 10) {
            return '0' + number.toString();
